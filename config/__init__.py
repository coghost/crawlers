...
class Conf(object):
    ...
    def init_type(self):
        ...
        _cfg.init('taobao.username', '', str)
        _cfg.init('taobao.password', '', str)
        _cfg.init('taobao.ua', '', str)

        _cfg.init('xici.end_date', '', str)
        _cfg.init('xici.last_index', 0, int)

        _cfg.init('kdl.last_index', 0, int)
        ...
        _cfg.init('dongao.dest', '', str)
        _cfg.init('dongao.password', '', str)  # 直接从web获取即可
